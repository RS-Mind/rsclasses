...
{
    public class SpawnBulletsEffect : MonoBehaviour // This is Pykess's. I take 0 credit for this code
    {
            ...
            for (int i = 0; i < this.gunToShootFrom.projectiles.Length; i++)
            {
                for (int j = 0; j < currentNumberOfProjectiles; j++)
                {
                    ...
                    if ((bool)typeof(Gun).InvokeMember("CheckIsMine", BindingFlags.Instance | BindingFlags.InvokeMethod | BindingFlags.NonPublic, null, this.gunToShootFrom, new object[] { }))
                    {
                        ...
                        GameObject gameObject = PhotonNetwork.Instantiate(this.gunToShootFrom.projectiles[i].objectToSpawn.gameObject.name, positionToShootFrom, Quaternion.Euler(directionToShootThisBullet), 0, null);

                        if (PhotonNetwork.OfflineMode)
                        {
                            spawnBulletMono.RPCA_Shoot(gameObject.GetComponent<PhotonView>().ViewID, currentNumberOfProjectiles, 1f, UnityEngine.Random.Range(0f, 1f), gunToShootFrom);
                            numShot++;
                        }
