...
namespace RSClasses
{
    public class MirrorBounce : MonoBehaviour
    {
        ...
        private void Start()
        {
            GetComponentInParent<ChildRPC>().childRPCsVector2Vector2IntInt.Add("MirrorBounce", DoHit);
            ...
            positive = this.transform.position.x > 0f;
        }
        ...
        private void FixedUpdate()
        {
            ...
            if (!view.IsMine || sinceBounce < 0.1f)
                return;
            if (this.transform.position.x != 0 && this.transform.position.x > 0f == positive) // Check if the bullet is still on the correct side of the screen
                return;
            RaycastHit2D raycastHit2D = default(RaycastHit2D);
            raycastHit2D.normal = Vector2.right;
            ...
            GetComponentInParent<ChildRPC>().CallFunction("MirrorBounce", raycastHit2D.point, raycastHit2D.normal, num, intData);
            sinceBounce = 0f;
        }
