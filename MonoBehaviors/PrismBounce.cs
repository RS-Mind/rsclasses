...
namespace RSClasses
{
    public class PrismBounce : MonoBehaviour
    {
        ...
        private void Start()
        {
            GetComponentInParent<ChildRPC>().childRPCsVector2Vector2IntInt.Add("PrismBounce", DoHit);
            ...
            positive = this.transform.position.y > 0f;
        }
        ...
        private void FixedUpdate()
        {
            ...
            if (!view.IsMine || sinceBounce < 0.1f)
                return;
            if (this.transform.position.y != 0 && this.transform.position.y > 0f == positive) // Check if the bullet is still on the correct side of the screen
                return;
            RaycastHit2D raycastHit2D = default(RaycastHit2D);
            raycastHit2D.normal = Vector2.up;
            ...
            GetComponentInParent<ChildRPC>().CallFunction("PrismBounce", raycastHit2D.point, raycastHit2D.normal, num, intData);
            sinceBounce = 0f;
        }
